...
import { FiGift, FiAward, FiArrowLeft } from 'react-icons/fi';
...
function PredictionMarket({ account, provider, contract, marketId }) {
    ...
    const [pricePoints, setPricePoints] = useState(() => {
        ...
        const stored = localStorage.getItem(`pricePoints-${marketId}`);
    ...
    // Function to add a new price point
    const addPricePoint = useCallback(async () => {
        if (!contracts.predictionMarket || !marketId) return;

        try {
            const marketInfo = await contracts.predictionMarket.getMarketInfo(marketId);
            const yesPrice = marketInfo[5];
            const formattedPrice = yesPrice.toString();
            
            setPricePoints(prev => {
                // Don't add if it's the same as the last price
                if (prev.length > 0 && prev[prev.length - 1].price === formattedPrice) {
                    return prev;
                }
                ...
                // Create new array with circular buffer logic
                let newPoints;
                if (prev.length >= 50) {
                    // Remove first element and add new one at the end
                    newPoints = [...prev.slice(1), newPoint];
                } else {
                    // Just add new point if under 50
                    newPoints = [...prev, newPoint];
                }

                // Save to localStorage
                localStorage.setItem(`pricePoints-${marketId}`, JSON.stringify(newPoints));
    ...
    // Initialize contracts and set up event listeners
    useEffect(() => {
        if (provider) {
            ...
            // Get initial price point
            if (predictionMarketContract && marketId) {
                addPricePoint();
            }
        }
    }, [provider, contract, marketId]);

    // Set up trade event listener
    useEffect(() => {
        if (!contracts.predictionMarket || !marketId) return;

        const handleTrade = async (tradeMarketId) => {
            if (tradeMarketId.toString() === marketId.toString()) {
                await addPricePoint();
            }
        };

        contracts.predictionMarket.on('Trade', handleTrade);

        return () => {
            contracts.predictionMarket.removeListener('Trade', handleTrade);
        };
    }, [contracts.predictionMarket, marketId, addPricePoint]);
    ...
    useEffect(() => {
        return () => {
            localStorage.removeItem(`pricePoints-${marketId}`);
        };
    }, [marketId]);
    ...
    return (
        <div className="container mx-auto px-4 py-8">
            ...
            <div className="mb-6 flex items-center justify-between">
                <Link
                    to="/markets"
                    className="group flex items-center gap-2 px-4 py-2 rounded-xl bg-gradient-to-r from-[#1a1f35] to-[#1a1f35] hover:from-purple-600 hover:to-blue-600 border border-purple-500/30 transition-all duration-300 hover:border-purple-500/50 hover:shadow-lg hover:shadow-purple-500/20"
                >
                    <FiArrowLeft className="w-5 h-5 text-purple-400 group-hover:text-white transition-colors duration-300" />
                    <span className="text-purple-400 group-hover:text-white font-medium transition-colors duration-300">Back to Markets</span>
                </Link>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                ...
                <div className="w-full">
                    <MarketInfo contract={contracts.predictionMarket} marketId={marketId} />
                </div>
            ...
            {/* Price Graph */}
            <PriceGraph pricePoints={pricePoints} />
